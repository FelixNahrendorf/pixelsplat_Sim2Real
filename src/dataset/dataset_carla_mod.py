...
CARLA_DATASET_ROOT = '/app/data/nuscenes_theo/Nuscenes/' # Change this to your data directory
...
class Dataset_CARLA(Dataset):
    ...
    def __init__(
        ...
        if (self.stage == 'train'):
            ...
            self.spawn_dirs =  [str_list_concat(spawns_dir, spawns_dir, 'step_0')  for spawns_dir in self.parent_dirs]
            ...
            self.parent_dirs = [CARLA_DATASET_ROOT + 'Town' + town + data_dir_naming for town in testing_towns]
            self.spawn_dirs =  [str_list_concat(spawns_dir, spawns_dir, 'step_0')  for spawns_dir in self.parent_dirs]
            self.spawn_dirs = list(itertools.chain.from_iterable(self.spawn_dirs))
            random.shuffle(self.spawn_dirs) 
            self.input_images = [spawn_dir + '/nuscenes/transforms/transforms_ego.json' for spawn_dir in self.spawn_dirs]
