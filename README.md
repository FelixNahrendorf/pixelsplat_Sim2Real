# pixelSplat_Sim2Real
This is the code of the extension of the pixelSplat model for performing Simulation-to-Reality zero-shot driving scene reconstruction. The model was trained on synthetic data ([SEED4D](https://seed4d.github.io) and modifications) only, followed by zero-shot testing on the [NuScenes](https://www.nuscenes.org/) dataset — a large real-world autonomous-driving dataset with synchronized surround-camera recordings. This work was part of my Master Thesis on "Real World Birds-Eye View Generation without Ground-Truth Supervision" at Technical University of Berlin, Germany (German Grade: 1.3). The key problem to solve was the generation of birds-eye views from six vehicle-outfacing cameras in absence of views from the top in real-time.

Two terms used throughout: **ego-views** are the images captured by the cameras mounted *on* the vehicle (the six outward-facing ring cameras — the model's only input at test time), while **exo-views** are external viewpoints looking *at* the scene from outside the car, including the elevated top-down cameras that form the birds-eye view. NuScenes provides ego-views only; there are no real exo-views to supervise against, which is the core difficulty this work addresses.

In order to do that, the following parts were built to extend and modify the base pixelSplat model:

- **Custom NuScenes & SEED4D dataloaders** — realigning camera coordinate frames from NuScenes convention to CARLA/SEED4D convention, normalizing intrinsics per sensor resolution, and resolving the coordinate-origin mismatch (NuScenes' global map coordinates vs. SEED4D's local scene-centered coordinates) that otherwise produces empty renderings.
- **Ego-exo target sampling** — feeding the six ego cameras as context while using elevated SEED4D exo-views (and a focused high-altitude subset) as birds-eye-view reconstruction targets, since no ground-truth top-down views exist for NuScenes.
